...
function UserBar({
	...
	const localAvatar = avatarUrl ?? null;
	const initials = username?.charAt(0).toUpperCase() ?? "?";

	return (
		<>
			<div className="h-14 bg-gray-900 px-3 flex items-center justify-between gap-1">
				{/* User Info – click to open profile modal */}
				<button
					onClick={() => setIsProfileOpen(true)}
					className="flex items-center gap-2 min-w-0 flex-1 rounded-lg py-1.5 hover:bg-gray-800 transition text-left group"
					title="Edit profile"
				>
					{/* Avatar */}
					...
					<div className="min-w-0">
						<p className="text-white text-sm font-medium truncate leading-tight group-hover:text-indigo-300 transition">
							{username}
						</p>
						{tag && discriminator && (
							<p className="text-gray-500 text-[10px] truncate leading-tight font-mono">
								{tag}#{discriminator}
							</p>
						)}
					</div>
